...
fn free_in_place_scope_fifo_runs_body_on_caller_and_chains_fifo_and_broadcast_work() {
    ...
    let outcome = rayon_core::in_place_scope_fifo(|scope| {
        assert_eq!(
            ...
            caller_thread,
            "rayon_core::in_place_scope_fifo should execute its body on the calling thread"
        );
        assert_eq!(
            rayon_core::current_thread_index(),
            None,
            "the in-place FIFO body itself should not be migrated to a Rayon worker"
        );
        assert_eq!(rayon_core::current_thread_has_pending_tasks(), None);
        assert_eq!(rayon_core::current_num_threads(), global_threads);
        ...
        for seed_record in seeds.iter().cloned() {
            ...
            rayon_core::ScopeFifo::spawn_fifo(scope, move |nested_scope| {
                ...
                let executing_index = rayon_core::current_thread_index()
                    .expect("FIFO parent work spawned by in_place_scope_fifo should run on a Rayon worker");
    ...
    assert!(
        ...
        "rayon_core::in_place_scope_fifo should be able to return caller-local non-Send data"
    );
    ...
    for record in &broadcast_records {
        ...
        assert!(
            record.pending_status_available,
            "broadcast work in in_place_scope_fifo should observe pending-task status"
        );
    }
...
#[test]
#[cfg_attr(any(target_os = "emscripten", target_family = "wasm"), ignore)]
fn free_in_place_scope_fifo_uses_current_single_worker_pool_and_preserves_fifo_order() {
    let pool = rayon_core::ThreadPoolBuilder::new()
        .num_threads(1)
        .thread_name(|index| format!("free-in-place-fifo-current-pool-worker-{index}"))
        .build()
    ...
    let summary = rayon_core::ThreadPool::scope(&pool, |_| {
        ...
        rayon_core::in_place_scope_fifo(|scope| {
            assert_eq!(
                rayon_core::current_thread_index(),
                Some(0),
                "rayon_core::in_place_scope_fifo called from a pool worker should keep its body on that worker"
            );
            assert_eq!(rayon_core::current_num_threads(), 1);
    ...
    assert_eq!(
        ...
        "FIFO jobs queued by the in-place body should execute in queue order on one worker"
    );
}

#[test]
#[cfg_attr(any(target_os = "emscripten", target_family = "wasm"), ignore)]
fn free_in_place_scope_fifo_propagates_body_panic_after_prequeued_fifo_jobs_finish_and_recovers() {
    let global_threads = rayon_core::current_num_threads();
    assert!(global_threads > 0);
    ...
    let panic_result = catch_unwind(AssertUnwindSafe(|| {
        rayon_core::in_place_scope_fifo(|scope| {
            ...
            panic!(
                "intentional rayon_core::in_place_scope_fifo body panic after scheduling FIFO work"
            );
        });
    }));

    let payload = panic_result
        .expect_err("a body panic should propagate out of rayon_core::in_place_scope_fifo");
    let panic_message = panic_payload_to_string(&*payload);

    assert!(
        panic_message.contains("in_place_scope_fifo body panic"),
...
#[test]
#[cfg_attr(any(target_os = "emscripten", target_family = "wasm"), ignore)]
fn free_in_place_scope_fifo_propagates_spawned_fifo_panic_and_later_reuses_pool() {
    let global_threads = rayon_core::current_num_threads();
    assert!(global_threads > 0);
    ...
    let panic_result = catch_unwind(AssertUnwindSafe(|| {
        let _: usize = rayon_core::in_place_scope_fifo(|scope| {
            rayon_core::ScopeFifo::spawn_fifo(scope, |_| {
                ...
                assert!(
                    rayon_core::current_thread_has_pending_tasks().is_some(),
                    "panicking FIFO work should observe worker-local pending-task status"
                );

                panic!(
                    "intentional spawned FIFO panic from rayon_core::in_place_scope_fifo worker {worker_index}"
                );
            });
    ...
    let payload = panic_result.expect_err(
        "a panic in FIFO work spawned by in_place_scope_fifo should propagate to the caller",
    );
    let panic_message = panic_payload_to_string(&*payload);

    assert!(
        panic_message.contains("intentional spawned FIFO panic from rayon_core::in_place_scope_fifo"),
        "unexpected propagated panic payload: {panic_message:?}"
    );
    ...
    for record in &recovery_seeds {
        assert_eq!(record.num_threads, global_threads);
        assert_eq!(record.current_index, Some(record.index));
        assert_eq!(record.seed, (record.index + 7) * (global_threads + 23));
    }

    let seed_by_index: Vec<_> = recovery_seeds.iter().map(|record| record.seed).collect();
        ...
        for seed_record in recovery_seeds.iter().cloned() {
            ...
            rayon_core::ScopeFifo::spawn_fifo(scope, move |nested_scope| {
                ...
                let parent_executing_index = rayon_core::current_thread_index()
                    .expect("recovery parent FIFO work should run on a Rayon worker");
                ...
                rayon_core::ScopeFifo::spawn_fifo(nested_scope, move |_| {
                    ...
                    let child_executing_index = rayon_core::current_thread_index()
                        .expect("recovery child FIFO work should run on a Rayon worker");
                    assert!(child_executing_index < global_threads);
