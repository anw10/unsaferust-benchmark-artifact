...
fn thread_pool_builder_start_handler_publishes_worker_state_used_by_broadcast_scope_async_and_exit()
{
    ...
    let builder = rayon_core::ThreadPoolBuilder::new()
        ...
        .thread_name(|index| format!("root-start-handler-worker-{index}"))
        .exit_handler({
    ...
    let builder = rayon_core::ThreadPoolBuilder::start_handler(builder, {
        ...
        move |index| {
            ...
            let seed = (index + 1) * (thread_count + 41);
    ...
    let mut start_events = recv_exact(&start_rx, thread_count, "start_handler events");
    ...
    for event in &start_events {
        let expected_name = format!("root-start-handler-worker-{}", event.index);
        ...
        assert_eq!(event.seed, (event.index + 1) * (thread_count + 41));
    }
    ...
    for (index, seed) in seed_by_index.iter().copied().enumerate() {
        assert_eq!(seed, (index + 1) * (thread_count + 41));
    }
    ...
    let mut observations = rayon_core::ThreadPool::broadcast(&pool, |context| {
        ...
        StartSeededObservation {
            index,
            num_threads,
            current_index: rayon_core::current_thread_index(),
            name: std::thread::current().name().map(str::to_owned),
            seed,
            derived: seed + index + num_threads * 3,
    ...
    for observation in &observations {
        let expected_name = format!("root-start-handler-worker-{}", observation.index);
        ...
        assert_eq!(
            observation.seed,
            (observation.index + 1) * (thread_count + 41)
        );
        assert_eq!(
            observation.derived,
            observation.seed + observation.index + thread_count * 3
        );
        assert!(
    ...
    let observation_by_index: BTreeMap<usize, StartSeededObservation> = observations
        .iter()
        .cloned()
    ...
    let expected_observation_sum: usize = observations.iter().map(|record| record.derived).sum();
    ...
    rayon_core::ThreadPool::spawn_broadcast(&pool, {
        ...
        move |context| {
            ...
            let (left, right) =
                rayon_core::join(move || seed + scoped_total, move || index + num_threads * 7);
    ...
    let mut async_records = recv_exact(
        &async_rx,
        thread_count,
        "spawn_broadcast work after start_handler",
    );
    ...
    for record in &async_records {
        ...
        assert_eq!(
            record.value,
            seed_by_index[record.index] + scoped_total + record.index + thread_count * 7
        );
        assert!(record.pending_status_available);
    }
    ...
    let mut exit_events = recv_exact(&exit_rx, thread_count, "exit_handler events");
    ...
    for event in &exit_events {
        ...
        assert_eq!(
            ...
            "exit handler should observe state published after start-seeded work"
        );
    }
...
#[test]
#[cfg_attr(any(target_os = "emscripten", target_family = "wasm"), ignore)]
fn thread_pool_builder_start_handler_panics_are_forwarded_and_start_seeded_pool_recovers() {
    ...
    let builder = rayon_core::ThreadPoolBuilder::new()
        .num_threads(thread_count)
        .thread_name(|index| format!("root-start-handler-panic-worker-{index}"))
        .panic_handler({
    ...
    let builder = rayon_core::ThreadPoolBuilder::start_handler(builder, {
        ...
        move |index| {
            ...
            start_tx
                ...
                .expect("start handler should report before intentional panic");
    ...
    let pool = rayon_core::ThreadPoolBuilder::build(builder)
        .expect("start_handler panics should be handled and the pool should still build");
    ...
    let mut start_attempts = recv_exact(
        &start_rx,
        thread_count,
        "start_handler attempts before panic",
    );
    ...
    for attempt in &start_attempts {
        let expected_name = format!("root-start-handler-panic-worker-{}", attempt.index);
    ...
    assert!(
        start_rx.recv_timeout(Duration::from_millis(100)).is_err(),
        "start_handler should run exactly once per worker even when one invocation panics"
    );

    let expected_panic_count = (0..thread_count).filter(|index| index % 2 == 1).count();
    let panic_events = recv_exact(
        &panic_rx,
        expected_panic_count,
        "panic_handler events for start_handler panics",
    );
    ...
    let expected_panic_records: BTreeSet<_> = start_attempts
        .iter()
        .filter(|attempt| attempt.index % 2 == 1)
    ...
    for record in &recovery_broadcast {
        ...
        assert!(
            ...
            "pool should accept broadcast work after a start_handler panic"
        );
    }
    ...
    let fifo_records = Mutex::new(Vec::<FifoRecoveryRecord>::new());
    ...
    let scope_return = rayon_core::ThreadPool::scope_fifo(&pool, |scope| {
        for record in recovery_broadcast.iter().cloned() {
            ...
            rayon_core::ScopeFifo::spawn_fifo(scope, move |_| {
                let executing_index = rayon_core::current_thread_index()
                    .expect("FIFO recovery work should run inside the custom pool");
    ...
    let exited: BTreeSet<_> =
        recv_exact(&exit_rx, thread_count, "exit_handler after start_handler panic")
            .into_iter()
            .collect();
    ...
    assert!(
        exit_rx.recv_timeout(Duration::from_millis(100)).is_err(),
        "each worker should exit exactly once"
    );
}