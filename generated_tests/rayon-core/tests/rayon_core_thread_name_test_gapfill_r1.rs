...
fn thread_name_fnmut_names_drive_lifecycle_broadcast_scope_async_and_exit_work() {
    ...
    let builder = rayon_core::ThreadPoolBuilder::thread_name(builder, {
        ...
        move |index| {
            ...
            let name = format!("root-thread-name-worker-{index}-call-{call_ordinal}");
    ...
    let builder = rayon_core::ThreadPoolBuilder::start_handler(builder, move |index| {
        start_tx
            ...
            .expect("start handler should report the worker name");
    });
    ...
    let builder = rayon_core::ThreadPoolBuilder::exit_handler(builder, {
        ...
        move |index| {
            exit_tx
                ...
                .expect("exit handler should report the worker name");
        }
    });
    ...
    assert_eq!(
        ...
        "FnMut state in the thread_name closure should advance once per call"
    );
    assert_eq!(
        name_calls
            .iter()
            ...
            .len(),
        name_calls.len(),
        "each generated thread name should be unique"
    );
    ...
    let scope_return = rayon_core::ThreadPool::scope(&pool, |scope| {
        for observation in observations.iter().cloned() {
            ...
            rayon_core::Scope::spawn(scope, move |_| {
                ...
                let origin_name_len = observation.name.len();
                let source_derived = observation.derived;
                let num_threads = observation.num_threads;

                let (left, right) = rayon_core::join(
                    move || source_derived + origin_name_len,
                    move || origin_index + running_index + num_threads,
                );
    ...
    for record in &scoped_records {
        ...
        assert_eq!(record.source_derived, origin.derived);
        assert_eq!(
            record.value,
            origin.derived + origin.name.len() + origin.index + record.running_index + origin.num_threads
        );
    }

    let scoped_by_origin = Arc::new(
        scoped_records
            .iter()
            .map(|record| (record.origin_index, record.value))
    ...
    rayon_core::ThreadPool::spawn_broadcast(&pool, {
        ...
        move |context| {
            ...
            let source_value = scoped_by_origin[&index];
            let name_len = name.len();
            let (left, right) =
                rayon_core::join(move || source_value + name_len, move || index + num_threads * 5);
    ...
    for record in &async_records {
        ...
        assert_eq!(record.source_value, scoped_by_origin[&record.index]);
        assert_eq!(
            record.value,
            record.source_value + record.name.len() + record.index + thread_count * 5
        );
    }
...
#[test]
#[cfg_attr(any(target_os = "emscripten", target_family = "wasm"), ignore)]
fn thread_name_is_passed_to_custom_spawn_handler_and_named_workers_recover_after_panics() {
    let thread_count = 3usize;
    let stack_size = 2 * 1024 * 1024usize;
    let expected_indices = expected_worker_indices(thread_count);

    let expected_names: Arc<BTreeMap<usize, String>> = Arc::new(
        (0..thread_count)
            .map(|index| {
                (
                    index,
                    format!("spawn-handler-thread-name-worker-{index}"),
                )
            })
            .collect(),
    );
    ...
    let builder = rayon_core::ThreadPoolBuilder::thread_name(builder, {
        let expected_names = Arc::clone(&expected_names);

        move |index| {
            expected_names
                .get(&index)
                .expect("thread_name closure should receive a known worker index")
                .clone()
        }
    });
    ...
    let builder = rayon_core::ThreadPoolBuilder::panic_handler(builder, {
        ...
        move |payload| {
            let event = classify_named_payload(&*payload);
    ...
    for seed_record in seeds.iter().cloned() {
        ...
        rayon_core::ThreadPool::spawn(&pool, move || {
            ...
            assert_eq!(expected_names.get(&executing_index), Some(&worker_name));
