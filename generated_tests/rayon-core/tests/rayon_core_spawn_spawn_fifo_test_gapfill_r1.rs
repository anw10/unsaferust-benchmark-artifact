...
fn classify_payload(payload: &(dyn Any + Send)) -> HandledPanic {
    if let Some(record) = payload.downcast_ref::<FifoPanicRecord>() {
        HandledPanic::Fifo(record.clone())
...
fn free_spawn_fifo_called_from_single_worker_pool_preserves_fifo_order_and_feeds_scoped_work() {
    let pool = rayon_core::ThreadPoolBuilder::new()
        .num_threads(1)
        .thread_name(|index| format!("free-spawn-fifo-single-current-pool-{index}"))
        .build()
    ...
    let expected_seed = seeds[0].seed;
    let (event_tx, event_rx) = mpsc::channel::<FifoEvent>();
    ...
    let schedule_report = rayon_core::ThreadPool::scope(&pool, move |_| {
        ...
        for queue_position in 0..scheduled_jobs {
            ...
            rayon_core::spawn_fifo(move || {
                let worker_index = rayon_core::current_thread_index()
                    .expect("free spawn_fifo work should run on the current custom pool");
                ...
                let (left, right) = rayon_core::join(
                    ...
                    move || worker_index + queue_position * 10,
                );
        ...
        ScheduleReport {
            ...
            scheduled_jobs,
            seed_checksum: expected_seed * scheduled_jobs,
        }
    });
    ...
    assert_eq!(schedule_report.seed_checksum, expected_seed * scheduled_jobs);
    ...
    assert_eq!(
        ...
        "free spawn_fifo jobs queued from the same worker should execute in FIFO order on one worker"
    );

    let expected_event_sum: usize = events.iter().map(|event| event.value).sum();
    let followups = Mutex::new(Vec::<ScopedFollowup>::new());
        ...
        for event in events.iter().cloned() {
            ...
            rayon_core::ScopeFifo::spawn_fifo(scope, move |_| {
                let executing_index = rayon_core::current_thread_index()
                    .expect("scoped FIFO follow-up should run on the custom pool");
    ...
    assert_eq!(scope_fifo_return, expected_event_sum);
    ...
    for record in &followups {
        ...
        assert_eq!(
            record.combined,
            record.source_value + record.queue_position + 1
        );
    }
    ...
    let confirmation = rayon_core::ThreadPool::broadcast(&pool, |context| {
        ...
        ConfirmationRecord {
            ...
            num_threads,
            current_index: rayon_core::current_thread_index(),
            total: expected_event_sum + expected_followup_sum + num_threads,
        }
    });
    ...
    assert_eq!(
        ...
        vec![ConfirmationRecord {
            ...
            total: expected_event_sum + expected_followup_sum + 1,
        }]
    );
}
...
fn free_spawn_fifo_detached_panics_use_current_pool_handler_and_recovery_work_succeeds() {
    ...
    for event in panic_events {
        match event {
            HandledPanic::Fifo(record) => panic_records.push(record),
    ...
    let (recovery_tx, recovery_rx) = mpsc::channel::<RecoveryRecord>();
    let recovery_tx_for_schedule = recovery_tx.clone();
    let panic_records_for_recovery = panic_records.clone();

    let recovery_schedule_report = rayon_core::ThreadPool::scope(&pool, move |_| {
        let scheduling_worker = rayon_core::current_thread_index()
    ...
    let recovery_by_origin: BTreeMap<usize, RecoveryRecord> = recovery_records
        .iter()
        .cloned()
        .map(|record| (record.origin_index, record))
        .collect();
    ...
    let scoped_return = rayon_core::ThreadPool::scope_fifo(&pool, |scope| {
